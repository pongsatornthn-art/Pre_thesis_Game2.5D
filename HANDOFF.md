...
1. ~~ระบบเล็งปืน + แขนถือปืน แบบ 2.5D billboard~~ — โค้ดเสร็จ อยู่ระหว่างเจ้าของจูนใน Editor
2. **[ใหม่] ระบบยิงปืนแบบ Alien Shooter** — เอาโหมดเล็ง FPS (คลิกขวาซูมกล้อง) ออก เปลี่ยนเป็น: ถือปืน = ยิงได้เลย, คลิกขวาค้าง = "นิ่งขึ้น" (กรวยกระสุนแคบ + เดินช้า) แทน — **โค้ดเสร็จแล้ว รอเจ้าของ unassign `vCamAiming` ใน Inspector + เทส**
...
### ส่วน C — ระบบยิงปืนแบบ Alien Shooter (ใหม่)

**1. ปิดกล้อง FPS zoom:** เลือกก้อนที่มี `AimCameraController` → ลาก `Vcam Aiming` ออก (เว้นว่าง) — ตัว guard ในโค้ดเช็ค null อยู่แล้ว พอ `vCamAiming` ว่าง กล้องจะไม่สลับซูมอีกเลย ยังคงแพนตามเมาส์เหมือนเดิม
2. กด Play ถือปืน → **คลิกซ้ายยิงได้ทันที** ไม่ต้องคลิกขวาก่อน
3. เช็คว่าตัวละครหันตามเมาส์ลื่นๆ ตลอดเวลาที่ถือปืน (ไม่กระตุก 8 ทิศแบบตอนถือดาบ)
4. กดคลิกขวาค้าง → เดินช้าลง + กรวยกระสุนแคบลง (ดูจาก crosshair ถ้ามี UI แสดงกรวย)
5. ปรับตัวเลขได้ที่ `PlayerCombat`:
   - `Steady Spread Multiplier` (default 0.35) — คลิกขวาค้างแล้วกรวยแคบแค่ไหน
   - `Steady Aim Speed Multiplier` (default 0.5) — คลิกขวาค้างแล้วเดินช้าแค่ไหน
6. ถ้ายิงกระสุนกระจายเยอะ/น้อยไปตอนไม่ได้คลิกขวา ปรับที่ `RangedWeaponData` ของปืนกระบอกนั้น (`maxSpreadAngle`, `focusTime`)

**ยังไม่ทำ:** ปุ่ม UI/ไอคอนบอกสถานะ "นิ่งขึ้น", แอนิเมชันท่ายืนนิ่งแยกจากท่ายิงปกติ (ตอนนี้ยังใช้ `Aim_Idle` เดิม)
...
- **[2026-09-16] Claude Code:** ไล่อ่านทุกสคริปต์บนตัวผู้เล่นแล้วสรุปเป็นไฟล์ใหม่ **[PLAYER_SYSTEMS.md](PLAYER_SYSTEMS.md)** (ระบบทั้งหมดแยกตามแหล่งที่มา เดิม/เรา/เพื่อน + ตารางปุ่ม + ของค้าง 7 ข้อ) · เจอสาเหตุบัค "จอบัคตอนคลิกขวา" = `AimPivotController.cs` ของเก่ายังสั่ง `Cursor.lockState = Locked` อยู่ → **ถอดการทำงานออกแล้ว** (เหลือคลาสเปล่ากัน Missing Script, เจ้าของลบ component ในซีนได้) และแก้ `CrosshairController.cs` ที่ยังล็อกเป้ากลางจอตอนคลิกขวา → ตามเมาส์ตลอดแล้ว
- **[2026-09-15] Claude Code (รอบ 2):** ทำระบบยิงปืนใหม่ตามที่เจ้าของขอ (แบบ Alien Shooter) — เอาโหมดเล็ง FPS (คลิกขวาซูมกล้อง, ต้องเล็งก่อนถึงยิงได้) ออกทั้งหมด เปลี่ยนเป็นถือปืน=ยิงได้เลย, คลิกขวาค้าง=นิ่งขึ้น (แคบกรวย+เดินช้า) แทน แก้ 3 ไฟล์: `PlayerCombat.cs` (เพิ่ม `HasRangedWeaponEquipped`, `steadySpreadMultiplier`, `steadyAimSpeedMultiplier`, ตัด gate ยิง/หันหน้า, รวม spread เป็น `MoveTowards` เส้นเดียว), `PlayerMovement.cs` (หันตามเมาส์เสมอ ไม่มีสาขากล้อง FPS แล้ว, ราว-vs-snap ใช้ชนิดอาวุธแทน isAiming, เดินช้าตอนนิ่งขึ้น), `PlayerAnimationController.cs` (ตัด isAiming ออกจากเงื่อนไขยิง, กันแย่ง AimX/AimZ ตอนถือปืน) → **ส่งต่อเจ้าของ: unassign `vCamAiming` ใน Inspector + เทส** (ดู "ส่วน C")
- **[2026-09-15] Claude Code (รอบ 1):** เจ้าของแจ้งว่า `HANDOFF.md` + สคริปต์ปืน/แขน 3 ไฟล์ (`ModularAimController.cs`, `WeaponSpriteRig.cs`, `DirectionalArm.cs`) หายจากดิสก์ — เช็คแล้วไฟล์เหล่านี้**ไม่เคย commit เข้า git** เลย (ยัง untracked) ส่วนเพื่อนร่วมทีม push 4 commit ใหม่เข้ามาและมีคน pull (fast-forward) พร้อมกัน คาดว่าโดน "Discard changes" ใน GitHub Desktop ลบไฟล์ untracked ทิ้งไปด้วย (ไม่ใช่จาก pull โดยตรง) → **กู้คืนไฟล์ทั้งหมดจากเนื้อหาที่คุยกันในบทสนทนา** (โค้ดตรงเวอร์ชันล่าสุดที่เจ้าของเทสอยู่) → แนะนำให้ commit ทันทีหลังกู้ ดูหัวข้อ "กันไฟล์หายซ้ำ"
